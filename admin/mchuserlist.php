...
if(request::post("action") == "delete") {
    ...
    $sqlStr = "DELETE FROM sys_mch_user WHERE id = '$userId'";
    $mysqlObj->executeUpdate($sqlStr);
    
    // 删除该用户的登录日志
    $sqlStr = "DELETE FROM sys_mch_user_log WHERE user_id = '$userId'";
    $mysqlObj->executeUpdate($sqlStr);
...
// 查询数据总数
$sqlStr_count = "SELECT COUNT(1) AS count FROM sys_mch_user AS log"
                    .$sqlStr_search;
...
// 查询数据列表
$sqlStr_list = "SELECT * FROM sys_mch_user"
                    .$sqlStr_search
                                        ...
                                        <?php
                                            ...
                                            foreach ($dataList as $key => $value) {
                                                ...
                                                if ($value["email_verify"] == 1) {
                                                    echo "<td>".$value["email"]." <span class='badge text-bg-success'>已验证</span></td>";
                                                } else {
                                                    echo "<td>".$value["email"]." <span class='badge text-bg-danger'>未验证</span></td>";
                                                }
                                                
                                                // 判断手机号码验证状态
                                                if ($value["telephone_verify"] == 1) {
                                                    echo "<td>".$value["telephone"]." <span class='badge text-bg-success'>已验证</span></td>";
                                                } else {
                                                    echo "<td>".$value["telephone"]." <span class='badge text-bg-danger'>未验证</span></td>";
                                                }
                                                ...
                                                echo "<td>
                                                        <a class='link-underline link-underline-opacity-0' href='/admin/editmchuser.php?id=".$value["id"]."'><i class='bi bi-pencil-square'></i> 修改</a>
                                                        ｜
